\begin{table*}[t]
\centering
\caption{Per-tile performance of the Mediterranean-trained XGBoost model. Mediterranean rows (S35W071--S37W072) use \textbf{spatial-block out-of-fold} predictions (\SI{10}{\kilo\meter} blocks, $K=5$, Section~\ref{sec:spatial_cv}); humid temperate rows (S38W072--S39W073) and the non-Chilean OOD rows (N10E105 Vietnam Mekong Delta; S24W069 Atacama) are direct application of the Mediterranean-trained model with no retraining or fold leakage. $\Delta\%$ = $(\text{RMSE}_{\text{corr}} - \text{RMSE}_{\text{raw}}) / \text{RMSE}_{\text{raw}} \times 100$; \textbf{negative values indicate RMSE reduction (improvement); positive values indicate degradation}, matching the sign convention used throughout the main text.}
\label{tab:per_tile}
\footnotesize
