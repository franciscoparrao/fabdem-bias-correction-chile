...
\begin{abstract}
Global 30-m digital elevation models such as FABDEM achieve aggregate vertical RMSE near \SI{2.5}{\meter}, but it remains unclear whether this average hides geographically heterogeneous bias that could be exploited for targeted regional correction, and whether such a correction transfers across distinct climate regimes. We address both questions using 135{,}350 ICESat-2 ATL08 v7 ground footprints over central-south Chile (34--39$^{\circ}$S) as cm-precision satellite LiDAR ground truth, spanning two contrasting regimes: Mediterranean watersheds (Mataquito and Maule) and humid temperate Valdivian rainforest (B\'io-B\'io and Araucan\'ia). For each footprint we engineer 33 features---geomorphometric (slope, curvature, openness, MRVBF, geomorphons), hydrological (HAND, TWI, flow accumulation, valley depth), and satellite-derived (Sentinel-1 RTC $\sigma^{0}$, Sentinel-2 NDVI/NDWI/NDMI/BSI)---and train XGBoost regression on the residual $h_{te} - \mathrm{fabdem}$ after EGM2008 datum correction, with Optuna hyperparameter optimisation and spatial-block cross-validation (\SI{10}{\kilo\meter} blocks, $n = 570$, $K = 5$ folds). Spatial-block cross-validation reduces RMSE from \SI{3.054}{\meter} (95\% CI: 2.95--3.18) to \SI{2.483}{\meter} (2.37--2.62)---a \SI{0.57}{\meter} absolute reduction ($-18.7\%$; Diebold--Mariano $p < 0.001$)---and eliminates a systematic $+\SI{1.0}{\meter}$ positive bias. Stratification reveals strongly differential gains: $-41.6\%$ in dense vegetation (NDVI $> 0.6$), $-33.8\%$ above \SI{3000}{\meter}, but only $-19.1\%$ in floodplains (HAND 2--\SI{10}{\meter}) where FABDEM is already accurate. Applying the Mediterranean-trained model to humid temperate forest \emph{without retraining} yields a further $-22.2\%$ RMSE reduction (4.946 $\to$ \SI{3.850}{\meter}; CIs 4.84--5.06 $\to$ 3.76--3.96), with SHAP attribution shifting from drainage features to 3D shape descriptors (openness\_negative, valley\_depth)---consistent with the model capturing terrain--error relationships that transfer between contrasting regimes rather than memorising regional patterns. We probe the boundary of this transferability by also applying the same model, without modification, to two further $1^{\circ}$ ATL08 datasets outside the Chilean training span---Vietnam Mekong Delta (tropical wet, $n=20{,}569$) and Atacama (hyperarid, $n=20{,}681$). In both regimes the correction \emph{degrades} performance (Vietnam $+5.8\%$ RMSE; Atacama $+54\%$ RMSE on a FABDEM that is already accurate at \SI{0.6}{\meter}), establishing empirically that the regional correction is bounded by the joint distribution of bias-driving features rather than universal. We release the corrected DEM as a Cloud-Optimized GeoTIFF (CC BY-NC-4.0, inherited from FABDEM), the 135k-footprint training dataset, trained model, and pipeline code as open artefacts.
\end{abstract}
...
%% - Cross-regime transferability is bounded: humid temperate Chile -22%; tropical wet (Vietnam) +5.8%; hyperarid (Atacama) +54%
%% - XGBoost outperforms MLP and TabNet baselines on the same spatial-CV folds; SAR features are dispensable in Mediterranean
...
\begin{enumerate}
\item \textbf{An honest regional benchmark for ML correction of FABDEM}: we reduce spatial-block cross-validated RMSE from \SI{3.054}{\meter} (95\% CI 2.95--3.18) to \SI{2.483}{\meter} (2.37--2.62)---an absolute reduction of \SI{0.57}{\meter} ($-18.7\%$) that is highly statistically significant (Diebold--Mariano $p < 0.001$)---and eliminate a systematic $+\SI{1.0}{\meter}$ positive bias.
...
Under spatial-block cross-validation on the Mediterranean dataset ($n = 77{,}501$ footprints, 570 unique \SI{10}{\kilo\meter} blocks, $K = 5$ folds), the XGBoost residual model reduces RMSE from \SI{3.054}{\meter} (95\% bootstrap CI 2.95--3.18) for raw FABDEM to \SI{2.483}{\meter} (CI 2.37--2.62) after correction---an absolute reduction of \SI{0.57}{\meter}, or \textbf{$-18.7\%$} in relative terms. The corresponding MAE drops from \SI{1.423}{\meter} to \SI{1.099}{\meter} ($-22.8\%$). The improvement is highly statistically significant: a Diebold--Mariano test on the per-footprint squared-error differential yields DM $= 31.2$ ($p < 0.001$), and a paired sign test finds that 50{,}025 of 77{,}501 footprints (64.5\%) move closer to the ATL08 reference after correction ($p < 0.001$, binomial test against the equal-improvement null). For comparison, applying naive random $K$-fold CV (which ignores autocorrelation) gives an over-optimistic RMSE of \SI{2.395}{\meter}---only about 4\% below the spatial-CV estimate, indicating that the \SI{10}{\kilo\meter} spatial-block construction successfully removes most of the within-track autocorrelation without sacrificing model capacity. Per-tile performance (Table~\ref{tab:per_tile}) shows the spatial-CV OOF improvement on the six Mediterranean training tiles ranging from $+10.8\%$ RMSE \emph{degradation} on the sample-sparse alpine tile S36W071 ($n = 1{,}987$ in the central Andes, where the model extrapolates) to $-22.6\%$ on S35W071 ($n = 7{,}916$); the spatial-CV fold-level RMSE varies between \SI{2.20}{\meter} and \SI{2.78}{\meter}, with the higher-error fold corresponding to a block containing dense alpine sample (Fig.~\ref{fig:per_tile}). Aggregated by regime, humid temperate tiles (S38--S39, Section~\ref{sec:ood_results}) reach $-22.2\%$ RMSE, the largest individual gain $-29.2\%$ at S39W072. Sensitivity analyses, ablations, and deep-learning baselines on this dataset are reported in Section~\ref{sec:sensitivity_baselines}.

\input{tables/T3_per_tile}
...
\input{tables/T4_stratification}
...
The central transferability claim of this paper is tested by applying the Mediterranean-trained XGBoost model---without retraining or fine-tuning---to the 57{,}849 humid temperate footprints in tiles S38W072--S39W073. The result is a reduction in RMSE from \SI{4.946}{\meter} (95\% CI 4.84--5.06) to \SI{3.850}{\meter} (CI 3.76--3.96), an absolute reduction of \SI{1.10}{\meter} or \textbf{$-22.2\%$} in relative terms (Fig.~\ref{fig:ood}). The reduction is statistically robust under formal testing: a Diebold--Mariano test yields DM $= 40.2$ with $p < 0.001$, and a paired sign test finds 33{,}080 of 57{,}849 footprints (57.2\%) improve under correction ($p \ll 0.001$). Per humid temperate tile, the relative improvement ranges from $-16.3\%$ (S39W073, dense Araucan\'ia coastal forest) to $-29.2\%$ (S39W072, Araucan\'ia valley and pre-Andean foothills). Although the corrected RMSE in humid temperate (\SI{3.85}{\meter}) remains substantially higher than in Mediterranean (\SI{2.48}{\meter})---reflecting the 60\% larger baseline error of FABDEM in dense temperate forest---the absolute reduction (\SI{1.10}{\meter}) is nearly twice the in-regime gain (\SI{0.57}{\meter}), so the regional model recovers a comparable fraction of recoverable error in both regimes.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/F5_ood_generalization.pdf}
\caption{Per-tile correction performance across four climate regimes. The horizontal axis is the raw FABDEM RMSE per tile (m); the vertical axis is the relative RMSE change from ML correction (positive = improvement; negative = degradation). Marker area is proportional to $\sqrt{n}$ footprints in the tile. A single Mediterranean-trained XGBoost model is applied to every tile without retraining; Mediterranean tiles (orange) use spatial-block out-of-fold predictions. Humid temperate Chile tiles (teal) transfer favourably ($-22.2\%$ aggregate); Vietnam Mekong Delta (green diamond, tropical wet) degrades marginally ($+5.8\%$) and Atacama (tan triangle, hyperarid) catastrophically ($+54\%$) on a FABDEM that is already accurate at \SI{0.6}{\meter}. The two negative OOD results jointly bound the transferability claim and motivate the regime-similarity interpretation of Section~\ref{sec:transferability}.}
\label{fig:ood}
\end{figure*}
...
\paragraph{Beyond Chile: bounding the transferability empirically.} The within-Chile result raises the natural question of how far the transferability extends. To probe the boundary directly we apply the same Mediterranean-trained model, without any modification, to two further $1^{\circ}$ ATL08 datasets selected for maximum contrast with the training regime: Vietnam Mekong Delta tile N10E105 ($n = 20{,}569$ footprints; tropical wet, mean elevation below \SI{50}{\meter}, the area benchmarked by \citet{Hawker2024Vietnam}) and Chilean Atacama tile S24W069 ($n = 20{,}681$; hyperarid, $>$\SI{2000}{\meter} elevation, near-zero NDVI). The headline finding inverts: in both regimes the correction \emph{degrades} performance. Vietnam Mekong raw FABDEM RMSE is \SI{1.091}{\meter}, corrected \SI{1.154}{\meter} ($+5.8\%$, only $42\%$ of footprints improve). Atacama raw FABDEM RMSE is already \SI{0.605}{\meter}---comparable to airborne LiDAR reference accuracy---and the correction inflates this to \SI{0.934}{\meter} ($+54\%$, $39\%$ improve). Stratified analysis (Table~\ref{tab:per_tile}) clarifies the mechanism rather than rescuing the result: in Atacama the degradation is uniform across slope and NDVI bands and catastrophic on the flat playa surface of the Salar de Atacama ($<1^{\circ}$ slope, $n = 1{,}020$, $+396\%$). In Vietnam a narrow NDVI window 0.1--0.5 ($n = 12{,}526$) shows neutral-to-marginal improvement ($\Delta \in [-6.4\%, -0.9\%]$), suggesting that the canopy--bias signal is partly preserved; but the dominant slope and hydrological features in the model's decision logic do not apply on the Mekong's flat alluvial substrate, and dense vegetation ($>0.5$ NDVI) and bare-water surfaces both degrade. These two negative results empirically bound the within-Chile transferability claim: the Mediterranean-trained regional correction extends to humid temperate forest, consistent with the hypothesis that the favourable transfer requires both Andean-style relief and a FABDEM positive canopy bias to be jointly present, and fails when either the relief substrate (Vietnam Delta) or the baseline bias (Atacama, where FABDEM is already accurate) departs from the training distribution. This is a post-hoc interpretation based on two negative tests; a definitive test would require designed experiments across a wider grid of relief--canopy combinations. Section~\ref{sec:transferability} develops the implications.
...
To support reviewer scrutiny of methodological choices we report three sensitivity analyses on the Mediterranean dataset: feature-group ablation, spatial-block size, and a tabular deep-learning baseline comparison. All use the locked-in hyperparameters of Table~\ref{tab:hyperparams} (no re-tuning) and the same spatial-CV partitioning of Section~\ref{sec:spatial_cv}.
...
\textbf{Tabular deep-learning baselines}. On the identical spatial-CV folds, a three-layer MLP achieves OOF RMSE \SI{2.578}{\meter} and TabNet \citep{Arik2021TabNet} \SI{2.606}{\meter}, both inferior to XGBoost's \SI{2.483}{\meter} by 3.8\% and 4.9\% respectively (Table~\ref{tab:dl_baseline}). For fairness we note that the XGBoost hyperparameters were Optuna-tuned over 100 trials whereas the MLP and TabNet were trained with literature-standard architectures (described in Section~\ref{sec:dl_baselines}) and no architecture search; an aggressive search might narrow the gap. Even so, the result is consistent with the broader tabular machine-learning literature \citep{ShwartzZiv2022TabularDL,Grinsztajn2022TabularDL}, which finds that gradient-boosted trees remain competitive with, or superior to, learned representations on tabular regression at sample sizes below approximately $10^{6}$. Both DL baselines do outperform raw FABDEM (\SI{3.054}{\meter}) by 16--19\%, so the choice of learner is a refinement question rather than a binary capability question; but XGBoost remains the most accurate option on this problem at this sample size and is also substantially cheaper to train and to interpret via SHAP (Section~\ref{sec:shap_results}).

\input{tables/T6_ablation}
\input{tables/T7_dl_baseline}
